 ...
 */
abstract class wpdr_AdminPageFramework_Factory_Model extends wpdr_AdminPageFramework_Factory_Router {
    ...
    protected function _loadFieldTypeDefinitions() {
        if (empty(self::$_aFieldTypeDefinitions)) {
            self::$_aFieldTypeDefinitions = wpdr_AdminPageFramework_FieldTypeRegistration::register(array(), $this->oProp->sClassName, $this->oMsg);
        }
    ...
    protected function _registerField(array $aField) {
        wpdr_AdminPageFramework_FieldTypeRegistration::_setFieldResources($aField, $this->oProp, $this->oResource);
        if ($aField['help']) {
    ...
    protected function _isValidationErrors() {
        if (isset($GLOBALS['awpdr_AdminPageFramework']['aFieldErrors']) && $GLOBALS['awpdr_AdminPageFramework']['aFieldErrors']) {
            return true;
        }
    ...
    public function _replyToSaveFieldErrors() {
        if (!isset($GLOBALS['awpdr_AdminPageFramework']['aFieldErrors'])) {
            return;
        }
        $this->oUtil->setTransient("apf_field_erros_" . get_current_user_id(), $GLOBALS['awpdr_AdminPageFramework']['aFieldErrors'], 300);
    }
    public function _replyToSaveNotices() {
        if (!isset($GLOBALS['awpdr_AdminPageFramework']['aNotices'])) {
            return;
        }
        if (empty($GLOBALS['awpdr_AdminPageFramework']['aNotices'])) {
            return;
        }
        $this->oUtil->setTransient('apf_notices_' . get_current_user_id(), $GLOBALS['awpdr_AdminPageFramework']['aNotices']);
    }
