 ...
 */
abstract class wpdr_AdminPageFramework_Factory_Controller extends wpdr_AdminPageFramework_Factory_View {
    ...
    public function setFieldErrors($aErrors) {
        $GLOBALS['awpdr_AdminPageFramework']['aFieldErrors'] = $this->oUtil->getElement($GLOBALS, array('awpdr_AdminPageFramework', 'aFieldErrors'), array());
        if (empty($GLOBALS['awpdr_AdminPageFramework']['aFieldErrors'])) {
            add_action('shutdown', array($this, '_replyToSaveFieldErrors'));
        }
        $_sID = md5($this->oProp->sClassName);
        $GLOBALS['awpdr_AdminPageFramework']['aFieldErrors'][$_sID] = isset($GLOBALS['awpdr_AdminPageFramework']['aFieldErrors'][$_sID]) ? $this->oUtil->uniteArrays($GLOBALS['awpdr_AdminPageFramework']['aFieldErrors'][$_sID], $aErrors) : $aErrors;
    }
    public function hasFieldError() {
        return isset($GLOBALS['awpdr_AdminPageFramework']['aFieldErrors'][md5($this->oProp->sClassName) ]);
    }
    public function setSettingNotice($sMessage, $sType = 'error', $asAttributes = array(), $bOverride = true) {
        $GLOBALS['awpdr_AdminPageFramework']['aNotices'] = $this->oUtil->getElement($GLOBALS, array('awpdr_AdminPageFramework', 'aNotices'), array());
        if (empty($GLOBALS['awpdr_AdminPageFramework']['aNotices'])) {
            add_action('shutdown', array($this, '_replyToSaveNotices'));
        }
        $_sID = md5(trim($sMessage));
        if ($bOverride || !isset($GLOBALS['awpdr_AdminPageFramework']['aNotices'][$_sID])) {
            ...
            $GLOBALS['awpdr_AdminPageFramework']['aNotices'][$_sID] = array('sMessage' => $sMessage, 'aAttributes' => $_aAttributes + array('class' => $sType, 'id' => $this->oProp->sClassName . '_' . $_sID,),);
        }
    }
    public function hasSettingNotice($sType = '') {
        $_aNotices = $this->oUtil->getElementAsArray($GLOBALS, array('awpdr_AdminPageFramework', 'aNotices'), array());
        if (!$sType) {
